...
def Send():
    ...
    def sender():
        ...
        port=8080
        #ssl_sock = ssl.wrap_socket(s, cert_reqs=ssl.CERT_NONE)  # Disable certificate validation, replace with proper certificate handling
        #ssl_sock.connect((host, port))
        s.bind((host,port))
        s.listen(1)
        print("The host: {}".format(host))
        print('is waiting for any incoming connections...')
        conn,addr=s.accept()
